Ansible Role: OS-RHEL8/RH_kdump/OS_build
=======================================================
# Description
本ロールは、RHEL8に関するカーネルダンプ設定についての情報の設定を行います。
...
OS-RHEL8/RH_kdump/OS_gatheringロールを利用します。
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL8/RH_kdump/OS_build
      VAR_RH_kdump:
      - path: /etc/kdump.conf
        text:
        - '# This file contains a series of commands to perform (in order) in the kdump'
...
- hosts: all
  gather_facts: true
  roles:
    - role: OS-RHEL8/RH_kdump/OS_gathering
  strategy: free
~~~
